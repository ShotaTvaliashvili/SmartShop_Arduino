...
/*
   Settling time (number of samples) and data filtering can be adjusted in 
the config.h file
   For calibration and storing the calibration value in eeprom, see 
example file "Calibration.ino"

   The update() function checks for new data and starts the next 
conversion. In order to acheive maximum effective
   sample rate, update() should be called at least as often as the HX711 
sample rate; >10Hz@10SPS, >80Hz@80SPS.
   If you have other time consuming code running (i.e. a graphical LCD), 
consider calling update() from an interrupt routine,
...
you want to fetch the calibration value from eeprom
  ...
  unsigned long stabilizingtime = 2000; // preciscion right after power-up 
...
void loop() {
  ...
  const int serialPrintInterval = 500; //increase value to slow down 
serial print activity
  ...
  // receive command from serial terminal, send 't' to initiate tare 
operation:
  ...
  content.toUpperCase();
  if (content.substring(1) == "63 CB 0B 0E") //change here the UID of the 
